...
class CodingAgent:
    ...
    async def run(self, input_text: str, options: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        async def bdd_tool_exec(tool_input):
            scenarios = await self.bdd.decompose(tool_input.get('requirement') or input_text)
            if on_stream:
                on_stream(StreamEvent(sessionId=options.get('sessionId') or 'default', conversationId=options.get('conversationId') or 'default', event={ 'id': self.gen_id('bdd_event'), 'role': 'assistant', 'type': 'bdd_event', 'data': { 'scenarios': scenarios } }, timestamp=self._now()))
            return { 'scenarios': scenarios }
        async def generate_tool_exec(tool_input):
            nonlocal final_project
            project = await self.generator.generate(tool_input.get('bdd'), {
        ...
        def forward_on_stream(evt: StreamEvent):
            ...
            if e and e.get('type') == 'tool_call_event' and e.get('data', {}).get('status') == 'end':
                ...
                if tool_name == 'decompose_bdd' and result and result.get('scenarios'):
                    on_stream and on_stream(StreamEvent(sessionId=evt.sessionId, conversationId=evt.conversationId, event={ 'id': self.gen_id('bdd_event'), 'role': 'assistant', 'type': 'bdd_event', 'data': { 'scenarios': result['scenarios'] } }, timestamp=self._now()))
